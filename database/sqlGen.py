...
def main():
    ...
    if args.version:
        # TODO: в будущем заменить pkg_resources тк он устарел
    ...
    engine = create_engine(args.url, encoding='utf-8')
...
def gen_command():
    command = [
        "{engine}://{user}:{password}@{host}:{port}/{db}?charset=utf8"
        .format(**config)
    ]
    for arg, val in args.items():
        if val:
            ...
            if prepare_value(val):
                command.append(prepare_value(val))
    return command
